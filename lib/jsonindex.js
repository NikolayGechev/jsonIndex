  ...
  for ( var i = 0; i < n; i ++ )
  {
    ...
    else if ( typeof options[i].multiple != 'undefined' )
    {
      ...
      for( var k = 0; k < p; k++ )
      {
        ...
        var jpath = jsonPath.eval( jsonObj, options[i].multiple[k], {resultType:"PATH"} );
        var m = elem.length;
        ...
        for (var j = 0; j < m; j ++) {
          ...
          index[j][k] = jsonPath.eval( jsonObj, jpath[j] );
          pathsIndex[j][k] = jsonPath.eval( jsonObj, jpath[j], {resultType:"PATH"} );
